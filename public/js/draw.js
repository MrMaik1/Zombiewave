...
function draw(){
  ...
  if(typeof drawWorldBounds==='function')drawWorldBounds(cx,cy);
  if(typeof drawArena==='function')drawArena(cx,cy);
  if(typeof drawMapObjects==='function')drawMapObjects(cx,cy);
...
function drawHUD(){
  ...
  const abg=document.getElementById('ab-gems');if(abg)abg.textContent=(currentUser?.questData?.gems||currentUser?.gems||0);
}
