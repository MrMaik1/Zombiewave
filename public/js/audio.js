...
function sfxShootWeapon(wpnId){
  if(!AC) return;
  switch(wpnId){
    case 'pistol': sfxNoise(.08,.14,1800); sfx(320,.06,'square',.05,80); break;
    case 'rifle':  sfxNoise(.05,.18,2400); sfx(280,.04,'square',.07,120); break;
    case 'shotgun': sfxNoise(.15,.25,800); sfx(90,.12,'sawtooth',.12,40); setTimeout(()=>sfxNoise(.1,.15,600),30); break;
    case 'sniper': sfxNoise(.04,.22,4000); sfx(180,.18,'square',.15,40); sfx(400,.05,'sine',.05); break;
    case 'rpg': sfx(80,.2,'sawtooth',.2,30); sfxNoise(.25,.3,300); break;
    case 'molotov': sfx(300,.1,'sine',.06,200); sfxNoise(.12,.1,1200); break;
    case 'minigun': sfxNoise(.04,.15,2200); sfx(400,.03,'square',.05); break;
    case 'kunai': sfx(1400,.06,'sine',.07,800); break;
    case 'crossbow': sfx(600,.04,'square',.08,200); sfxNoise(.06,.08,800); break;
    case 'flamethrower': sfxNoise(.05,.12,300); sfx(80,.05,'sawtooth',.06,60); break;
    case 'grenade': sfx(120,.15,'sawtooth',.12,60); sfxNoise(.1,.1,400); break;
    default: sfxNoise(.06,.12,1800);
  }
}
