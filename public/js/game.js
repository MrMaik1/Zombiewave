...
function initGame(isMP=false) {
  ...
  const charId    = u.activeChar || 'ghost';
  const charDef   = (typeof getCharacter === 'function') ? getCharacter(charId) : null;
  const cStat     = charDef ? charDef.stats : {speedMul:1,hpMul:1,shieldMul:1,dmgMul:1};
  const isNinja   = charId === 'ninja';
  const isBerserker = charId === 'berserker';

  // ── Class damage bonus ───────────────────────────────
  const classId    = u.activeClass || null;
  const classDmgBn = (classId && typeof getClassDmgBonus === 'function') ? getClassDmgBonus(classId) : 0;

  // Combined multipliers
  const baseHp    = Math.round((100 + perma.hp*25) * cStat.hpMul);
  const baseSpeed = 2.4 * (1+perma.speed*0.1) * cStat.speedMul;
  const totalDmgMul = (1+perma.dmg*0.15) * cStat.dmgMul * (1+classDmgBn);

  G={
    camX:0,camY:0,px:400,py:300,
    playerHp:baseHp,playerMaxHp:baseHp,
    playerSpeed:baseSpeed,
    facing:0,dashActive:false,dashTimer:0,dashCd:0,hasDash:false,
    dashCharges:isNinja?2:1, dashMaxCharges:isNinja?2:1,
    shield:false,shieldMul:cStat.shieldMul||1,
    regen:0,regenTick:0,invincible:0,godMode:false,adminMingunNoCool:false,
    ownedWpns:(u.weapons||['pistol']).filter(w=>WPN[w]).map(w=>({
      ...JSON.parse(JSON.stringify(WPN[w])),id:w,
      dmg:Math.round(20*(WPN[w].dmgMul||1)*totalDmgMul),
    ...
    surviveTimer:0,levelCompleted:false,
    levelGoal:(LEVELS.find(l=>l.id===selectedLevel)||LEVELS[0]).survivalGoal||300,
    guardianLvl:u.guardianShopLvl||0,guardians:[],guardianAngle:0,
    guardianVisible:true,guardianTimer:0,
    molotovDurMul:1,
    isMP,mpSnapshot:null,
    // Char/class state
    activeCharId:charId, activeClassId:classId,
    _berserkerActive:isBerserker, _berserkerBaseDmg:null,
    _shadowInvis:false,_shadowInvisTimer:0,_shadowFirstShot:false,
    _medicHealTick:0, _vampireKillCount:0,
    turrets:[],
  };

  // Passive regen bonuses from char specials
  if (charId==='sentinel')     G.regen += 1.5;
  if (charId==='guardian_char') G.regen += 2;
  if (classId==='medic') {/* medic heals via applyClassOnTick */}
...
function update(dt){
  ...
  // Keyboard + joystick (touch) movement
  let mx = (window._touchMoveX || 0);
  let my = (window._touchMoveY || 0);
  ...
  G.bullets.forEach(b=>{
    ...
    targets.forEach(e=>{
      ...
      if(dist2(b.x,b.y,e.x,e.y)<(e.size+b.size)**2){
        // Shadow: first shot after invis = 2x crit (lv5)
        let finalDmg = b.dmg;
        if(G._shadowFirstShot){G._shadowFirstShot=false;finalDmg*=2;spawnFT(e.x,e.y-e.size-10,'CRIT!','#1abc9c');}
        e.hp-=finalDmg;b.hits++;spawnBlood(e.x,e.y,6);spawnFT(e.x,e.y-e.size,'-'+finalDmg,'#ff6b5b');sfxHit();
        // Class on-hit hook (lifesteal, pyro ignite, etc.)
        if(G.activeClassId && typeof applyClassOnHit==='function') applyClassOnHit(G.activeClassId,e,finalDmg);
        // Report hit to server in MP
        if(G.isMP&&socket)socket.emit('game:hit',{enemyId:e.isBoss?'boss':e.id,dmg:finalDmg});
  ...
  // ── Class per-tick effects ───────────────────────────
  if(G.activeClassId && typeof applyClassOnTick==='function') applyClassOnTick(G.activeClassId,dt);
  // ── Berserker rage: scale damage with missing HP ─────
  if(G._berserkerActive) updateBerserkerRage();
  // ── Ninja double-dash recharge ───────────────────────
  if(G.dashCd<=0 && G.dashCharges < G.dashMaxCharges) G.dashCharges = G.dashMaxCharges;
}
...
function killEnemy(e){
  if(e.isBoss){
    ...
    // Class on-kill (boss counts)
    if(G.activeClassId && typeof applyClassOnKill==='function') applyClassOnKill(G.activeClassId,e);
  } else {
    spawnBlood(e.x,e.y,8);sfxDie();G.kills++;e.hp=-1;G.earnedCoins+=1;checkUpgrade();
    // Class on-kill hook
    if(G.activeClassId && typeof applyClassOnKill==='function') applyClassOnKill(G.activeClassId,e);
    // Berserker: update rage damage
    if(G._berserkerActive) updateBerserkerRage();
    if(e.explosive){
      sfxExplosion();
...
function startSoloGame(){
  ...
  document.getElementById('mp-player-bar').style.display='none';
  // Touch controls
  if(window.currentDevice==='mobile'||window.currentDevice==='tablet'){
    const tui=document.getElementById('touch-ui');
    if(tui) tui.style.display='block';
    if(typeof buildTouchWpnRow==='function') setTimeout(buildTouchWpnRow,100);
    if(typeof initTouchControls==='function') initTouchControls();
  }
  if(typeof positionAdminPanels==='function') positionAdminPanels();
...
// Berserker: the lower the HP, the more damage
function updateBerserkerRage() {
  if (!G || !G._berserkerActive) return;
  const hpPct = G.playerHp / G.playerMaxHp;
  // At full HP: no bonus. At 10% HP: +80% bonus.
  const rageMul = 1 + (1 - hpPct) * 0.8;
  G.ownedWpns.forEach(w => {
    if (G._berserkerBaseDmg === null) {
      // Store base dmg first time
      G._berserkerBaseDmg = {};
      G.ownedWpns.forEach(ww => G._berserkerBaseDmg[ww.id] = ww.dmg);
    }
    const base = G._berserkerBaseDmg[w.id];
    if (base !== undefined) w.dmg = Math.round(base * rageMul);
  });
}

// Ninja: allow dash if we still have charges (overrides normal dashCd check)
function ninjaCanDash() {
  if (!G) return false;
  return G.dashCharges > 0;
}

// Update class HUD badge during gameplay
function updateClassHUD() {
  const hud = document.getElementById('class-hud');
  if (!hud) return;
  const classId = window.G?.activeClassId || currentUser?.activeClass;
  if (!classId || !window.gameRunning) { hud.style.display = 'none'; return; }
  const cls = (typeof getClassDef === 'function') ? getClassDef(classId) : null;
  if (!cls) { hud.style.display = 'none'; return; }

  const lvl  = (typeof getClassLevel === 'function') ? getClassLevel(classId) : 0;
  const xp   = (typeof getClassXP    === 'function') ? getClassXP(classId)    : 0;
  const nextXp = lvl < cls.levels.length - 1 ? cls.levels[lvl + 1].xpRequired : null;
  const pct    = nextXp ? Math.min(100, Math.round(xp / nextXp * 100)) : 100;

  hud.style.display = 'block';
  hud.innerHTML = `
    <div class="class-hud-badge" style="border-color:${cls.color};">
      <span>${cls.icon}</span>
      <span style="color:${cls.color};font-size:.78rem;">${cls.name} Lv${lvl+1}</span>
    </div>
    <div class="class-xp-bar">
      <div class="class-xp-fill" style="width:${pct}%;background:${cls.color};"></div>
    </div>`;
}
